...
describe('Custom Manifests', () => {
  it('should load and use custom manifest files', async () => {
    ...
    const customManifestPath = join(__dirname, '../../test/fixtures/custom-manifest.json');
    ...
    const hasCustomMessage = result.messages.some(msg => 
  ...
  it('should prioritize custom replacements over built-in ones', async () => {
    ...
    const resultWithCustom = await runReplacements(fileSystem, [customManifestPath]);
    ...
    expect(resultWithCustom.messages.length).toBeGreaterThanOrEqual(resultWithoutCustom.messages.length);
  });

  it('should load multiple manifest files', async () => {
    const testDir = join(__dirname, '../../test/fixtures/basic-chalk');
    const fileSystem = new LocalFileSystem(testDir);
    const manifest1Path = join(__dirname, '../../test/fixtures/custom-manifest.json');
    const manifest2Path = join(__dirname, '../../test/fixtures/custom-manifest-2.json');

    const result = await runReplacements(fileSystem, [manifest1Path, manifest2Path]);

    // Should have messages from both manifests
    expect(result.messages.length).toBeGreaterThan(0);
    
    // Check that replacements from both manifests are included
    const hasChalkMessage = result.messages.some(msg => msg.message.includes('chalk'));
    const hasExpressMessage = result.messages.some(msg => msg.message.includes('express'));
